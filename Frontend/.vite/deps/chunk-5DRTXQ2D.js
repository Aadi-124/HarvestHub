...
function useMediaQueryOld(query, defaultMatches, matchMedia, ssrMatchMedia, noSsr) {
  const [match2, setMatch] = React26.useState(() => {
    if (noSsr && matchMedia) {
...
function useMediaQueryNew(query, defaultMatches, matchMedia, ssrMatchMedia, noSsr) {
  const getDefaultSnapshot = React26.useCallback(() => defaultMatches, [defaultMatches]);
  const getServerSnapshot = React26.useMemo(() => {
    if (noSsr && matchMedia) {
      return () => matchMedia(query).matches;
    }
  ...
  const [getSnapshot, subscribe] = React26.useMemo(() => {
    if (matchMedia === null) {
      return [getDefaultSnapshot, () => () => {
      }];
    }
...
@mui/private-theming/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-5DRTXQ2D.js.map
